...
class Resource:
    ...
    def __init__(self, access_token: str):
        ...
        self._accountid: str = str(self._decoded_token["acc_id"])
        self._userid: str = str(self._decoded_token["user_id"])
...
def date_diff(start, end):
    ...
    """
    fmt = "%Y%m%dT%H:%M:%SZ"
    start_ts = datetime.datetime.strptime(start, fmt).timestamp()
    end_ts = datetime.datetime.strptime(end, fmt).timestamp()
    return end_ts - start_ts > 0
